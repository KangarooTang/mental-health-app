...
# 用于图表的按天聚合（不依赖 user_tracker 的风险计算）
...
if records and daily_data:
    ...
    with st.sidebar.expander("📖 风险分数如何计算？"):
        st.markdown("""
        **风险分数** = 负面比例×40% + 恶化趋势×30% + 连续负面天数×20% + 危机信号×10%
        
        - **负面比例**：近14天中负面天数占比
        - **恶化趋势**：第二周负面比例 − 第一周负面比例（正值表示恶化）
        - **连续负面天数**：从今天往前数连续出现负面情绪的天数（上限7天）
        - **危机信号**：近14天是否出现自杀、自伤等极端表达（1=有，0=无）
...
if len(daily_data) > 0:
    ...
    report_data = {
        ...
        'emotion_counts': total_emotion_counts,  # 改为累计统计
